...
class WeiboContent:
    ...
    def getuserinfo(self,(user_list,info_list)):
        ...
        for user_info in user_list:
            ...
            time.sleep(1)
        print (userinfo_list)
        feature = pd.DataFrame(userinfo_list, columns=["gender", "birth","constellation", "location"])
        print (feature)

        feature1 =  feature[(feature["gender"]=="男")|(feature["gender"]=="女")]#去掉性别不为男女的部分
        print (feature1)
        #feature1 = feature1.reindex(range(0, 5212))
        print (info_list)
        return feature1, info_list

    def dataanaly(self,(feature, info_list)):
        feature.to_excel('F:/learning/weibo/feature.xlsx',encoding='utf_8_sig')
        ...
        print (text)
        labels = 'female', 'male'
        ax = feature_data["gender"].value_counts(normalize=True).plot.pie(title="gender distribution", labels=labels,
                                                                      autopct='%1.2f%%')

        plt.savefig("F:/learning/weibo/gender distribution.jpg", dpi=200)  # 不显示坐标轴
        plt.show()

        return text,feature_data
